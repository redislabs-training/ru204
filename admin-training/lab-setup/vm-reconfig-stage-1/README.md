...
SKIP these steps except...

### Get DNS config from the GCR image.

1. Return to SSH from GCP console so you're using your ***GCP account***.
...
2. Start node containers and create a cluster so nodes download and you test DNS resolves node and cluster names.
...
```
3. Stop and remove node containers. Removing them forces manual restart on new VMs. Otherwise, clusters will be misconfigured and cluster names will not resolve by DNS properly.

```bash
sudo docker stop n1 n2 n3
sudo docker rm n1 n2 n3
 
```

4. Return to VNC shell. Remove the ***known_hosts*** file.
...
5. Create a snapshot of the VM called ***admin-training-stage-1***.

6. Create an image from the snapshot called ***admin-training-stage-1***.
