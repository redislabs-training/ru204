...
public class SiteStatsDaoRedisImpl implements SiteStatsDao {
    ...
    // Consider a transaction on a single key.
    // TODO: In the optimizations pattern, maybe consider using strings?
    // - If you're always using the entire structure
    ...
    private void updateOptimized(Jedis jedis, String key, MeterReading reading) {
        try (Transaction t = jedis.multi()) {
            ...
            this.compareAndUpdateScript.updateIfGreater(t, key, SiteStats.minWhField,
                    reading.getWhGenerated());
